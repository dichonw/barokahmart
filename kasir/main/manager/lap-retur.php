...
$min_tanggal = mysqli_fetch_array(mysqli_query($koneksi, "SELECT MIN(tgl_retur) AS min_tanggal FROM tabel_retur"));
$max_tanggal = mysqli_fetch_array(mysqli_query($koneksi, "SELECT MAX(tgl_retur) AS max_tanggal FROM tabel_retur"));
?>

<form action="?menu=retur" method="post" name="postform">
  <div class="row">
    <div class="col-sm-4">
      <!------TABEL---------->
      <div class="panel-body panel-warning">
        <h4 class="box-header"><i class="fas fa-luggage-cart"></i> Data Retur</h4>
    ...
    <p>
      ...
      if (isset($_POST['cari'])) {
        ...
        if (empty($tanggal_awal) and empty($tanggal_akhir)) {
          ...
          $query = mysqli_query($koneksi, "SELECT * FROM tabel_retur");
          $jumlah = mysqli_fetch_array(mysqli_query($koneksi, "SELECT SUM(total_retur) AS total FROM tabel_retur"));
        } else {
      ?><i><b>Data retur : </b> Pencarian dari tanggal <b><?php echo $_POST['tanggal_awal'] ?></b> sampai dengan tanggal <b><?php echo $_POST['tanggal_akhir'] ?></b></i><?php

                                                                                                                                                                      $query = mysqli_query($koneksi, "SELECT * FROM tabel_retur WHERE tgl_retur BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                                                                                                                                                                      $jumlah = mysqli_fetch_array(mysqli_query($koneksi, "SELECT SUM(total_retur) AS total FROM tabel_retur WHERE tgl_retur BETWEEN '$tanggal_awal' AND '$tanggal_akhir'"));
                                                                                                                                                                    }
                                                                                                                                                                      ?>
    </p>
        ...
        <table id="demo-export" class="table table-bordered" style="font-size:11px">
          ...
          ?>
            <tr>
              ...
              <td><?php echo $row['no_faktur_retur']; ?></td>
              ...
              <td><?php echo number_format($row['total_retur'], 2, ',', '.'); ?></td>
              <td>
                <?php $c = mysqli_query($koneksi, "SELECT * FROM tabel_barang, tabel_rinci_retur WHERE tabel_barang.kd_barang = tabel_rinci_retur.kd_barang AND tabel_rinci_retur.no_faktur_retur = '$row[no_faktur_retur]' ");
